...
@objc protocol FBClusteringManagerDelegate {
    
    @objc optional func cellSizeFactorForCoordinator(_ coordinator:FBClusteringManager) -> CGFloat
...
class FBClusteringManager : NSObject {
    ...
    func deleteAllAnnotations(){
        lock.lock()
        tree?.deleteAllAnnotations()
        tree = FBQuadTree()
        lock.unlock()
        
    }
    
    func clusteredAnnotationsWithinMapRect(_ rect:MKMapRect, withZoomScale zoomScale:Double) -> [MKAnnotation]{
        
        
        var cellSize:CGFloat = FBClusteringManager.FBCellSizeForZoomScale(MKZoomScale(zoomScale))
    ...
    func displayAnnotations(_ annotations: [MKAnnotation], onMapView mapView:MKMapView){
    ...
    class func FBZoomScaleToZoomLevel(_ scale:MKZoomScale) -> Int{
    ...
    class func FBCellSizeForZoomScale(_ zoomScale:MKZoomScale) -> CGFloat {
